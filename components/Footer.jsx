...
const Footer = ({ dictionary, lang }) => {
    ...
    const linkSections = [
        {
            title: dictionary?.sections?.products?.title || "PRODUCTS",
            links: (dictionary?.sections?.products?.items || ["Earphones", "Headphones", "Smartphones", "Laptops"]).map(item => ({
                text: item,
                path: `/${lang}/shop`,
                icon: null
            }))
        ...
        {
            title: dictionary?.sections?.website?.title || "WEBSITE",
            links: [
                { text: dictionary?.sections?.website?.home || "Home", path: `/${lang}`, icon: null },
                { text: dictionary?.sections?.website?.privacy || "Privacy Policy", path: `/${lang}`, icon: null },
                { text: dictionary?.sections?.website?.plus || "Become Plus Member", path: `/${lang}/pricing`, icon: null },
                { text: dictionary?.sections?.website?.createStore || "Create Your Store", path: `/${lang}/create-store`, icon: null },
            ]
        },
        {
            title: dictionary?.sections?.contact?.title || "CONTACT",
            links: [
                { text: dictionary?.sections?.contact?.email || "sales@databayt.org", path: 'mailto:sales@databayt.org', icon: MailIcon },
                { text: dictionary?.sections?.contact?.phone || "+966 55 772 1603", path: 'tel:+966557721603', icon: PhoneIcon },
                { text: dictionary?.sections?.contact?.location || "Riyadh, Saudi Arabia", path: `/${lang}`, icon: MapPinIcon }
            ]
        }
    ];
        ...
        <footer className="mx-6 bg-white">
            <div className="max-w-7xl mx-auto">
                <div className="flex flex-col md:flex-row items-start justify-between gap-10 py-10 border-b border-slate-500/30 text-slate-500">
                    <div>
                        <Link href={`/${lang}`} className="text-4xl font-semibold text-slate-700">
                            <span className="text-green-600">go</span>cart<span className="text-green-600 text-5xl leading-0">.</span>
                        </Link>
                        <p className="max-w-[410px] mt-6 text-sm">{dictionary?.description || "Welcome to gocart, your ultimate destination for the latest and smartest gadgets. From smartphones and smartwatches to essential accessories, we bring you the best in innovation — all in one place."}</p>
                ...
                <p className="py-4 text-sm text-slate-500">
                    {dictionary?.copyright || "Copyright 2025 © gocart All Right Reserved."}
                </p>
            </div>
        </footer>
    );
